...
def estimate_head_pose(face_img):
    ...
    angles = head_pose_res['angle_y_fc'][0], head_pose_res['angle_p_fc'][0], head_pose_res['angle_r_fc'][0]
    return angles

def start_live_processing():
    cap = cv2.VideoCapture('http://192.168.29.38:8080/video')  # Connect to phone's camera
            ...
            for detection in detections[0][0]:
                if detection[2] > 0.5:  # Confidence threshold
                    ...
                    if face_img.size != 0:  # Check if face_img is not empty
                        ...
                        if best_match_score > 0.50:  # Threshold for recognition
